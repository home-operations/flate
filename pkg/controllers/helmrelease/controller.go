...
type Controller struct {
	*base.Controller
	...
	// HelmRelease valuesFrom Secret targets that are generated by
	// in-cluster secret controllers and therefore cannot exist in an
	// offline render.
...
type ReconcileOptions struct {
	...
	// AllowMissingSecrets soft-skips non-optional valuesFrom Secret refs
	// that point at known in-cluster secret-controller outputs.
	AllowMissingSecrets bool
}
...
func (c *Controller) reconcile(ctx context.Context, hr *manifest.HelmRelease) error {
	...
	if c.allowMissingSecrets {
		c.materializeGeneratedValuesSecrets(hr)
	}
...
func (c *Controller) materializeGeneratedValuesSecrets(hr *manifest.HelmRelease) {
	for _, ref := range hr.ValuesFrom {
		if ref.Optional || ref.Kind != manifest.KindSecret {
			continue
		}
		secretID := manifest.NamedResource{
			Kind: manifest.KindSecret, Namespace: hr.Namespace, Name: ref.Name,
		}
		existing, exists := store.GetByName[*manifest.Secret](c.Store, secretID.Kind, secretID.Namespace, secretID.Name)
		if exists && hasValuesKey(existing, ref.GetValuesKey()) {
			continue
		}
		if c.existence != nil && c.existence.IsFileIndexed(secretID) {
			continue
		}
		producer, ok := c.generatedSecretProducer(secretID)
		if !ok {
			continue
		}
		synthetic, ok := secretgen.FromRaw(producer)
		if !ok {
			continue
		}
		if exists {
			if !secretgen.IsPlaceholderSecret(existing) {
				continue
			}
			synthetic = secretgen.Merge(existing, synthetic)
		}
		synthetic = secretgen.EnsureKey(synthetic, ref.GetValuesKey())
		c.Store.AddObject(synthetic)
		slog.Debug("helmrelease: synthesized generated valuesFrom Secret",
			"id", hr.Named().String(),
			"secret", secretID.String(),
			"producer", producer.Named().String())
	}
}

func (c *Controller) generatedSecretProducer(secretID manifest.NamedResource) (*manifest.RawObject, bool) {
	for _, obj := range c.Store.ListObjects("") {
		raw, ok := obj.(*manifest.RawObject)
		if !ok || raw.Namespace != secretID.Namespace {
			continue
		}
		if secretgen.Produces(raw, secretID.Name) {
			return raw, true
		}
	}
	return nil, false
...
func hasValuesKey(secret *manifest.Secret, key string) bool {
	if key == "" {
		return false
	}
