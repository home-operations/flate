...
func (c *Controller) reconcile(ctx context.Context, hr *manifest.HelmRelease) error {
	...
	if c.allowMissingSecrets {
		if reason, ok := c.unrenderableValuesSecret(hr); ok {
			return fmt.Errorf("%w: %s", manifest.ErrSourceSkipped, reason)
		}
	}
...
func (c *Controller) unrenderableValuesSecret(hr *manifest.HelmRelease) (string, bool) {
	for _, ref := range hr.ValuesFrom {
		...
		if _, ok := store.GetByName[*manifest.Secret](c.Store, secretID.Kind, secretID.Namespace, secretID.Name); ok {
			continue
		}
		if c.existence != nil && c.existence.IsFileIndexed(secretID) {
			continue
		}
		if producer, ok := c.generatedSecretProducer(secretID); ok {
			return fmt.Sprintf("valuesFrom Secret %s is generated by %s and is unavailable offline",
				secretID.NamespacedName(), producer.String()), true
		}
	}
	return "", false
}

func (c *Controller) generatedSecretProducer(secretID manifest.NamedResource) (manifest.NamedResource, bool) {
	for _, obj := range c.Store.ListObjects("") {
		raw, ok := obj.(*manifest.RawObject)
		if !ok || raw.Namespace != secretID.Namespace {
			continue
		}
		if rawProducesSecret(raw, secretID.Name) {
...
func rawProducesSecret(raw *manifest.RawObject, name string) bool {
	switch raw.Kind {
	case "ExternalSecret":
