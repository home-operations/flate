...
`get ks` and `get hr` accept `-l/--selector key=value` for label filtering. `diff` accepts `--strip-attr <key>` (repeatable) to drop annotation/label keys before comparison; the default set covers chart-bump noise (`helm.sh/chart`, `checksum/config`, `app.kubernetes.io/version`, `chart`). Helm template flags are available on every reconcile-running subcommand because flate reconciles the full graph before filtering output. Reconcile-running subcommands accept `--allow-missing-secrets` to soft-skip source auth Secrets and synthesize placeholder generated Secrets that only exist in the live cluster — see [Behaviors](#behaviors).
...
**`--allow-missing-secrets`** — off by default. When set, a source whose auth `secretRef` is missing or PLACEHOLDER-wiped marks `Ready / "skipped: …"` instead of `Failed`, and consumers (KS `sourceRef`, HR `chartRef`) propagate the skip so `flate test` reports SKIPPED rather than a cascade of FAILED. Rendered ExternalSecret and SealedSecret resources synthesize placeholder Secret data, so HelmRelease `valuesFrom` can render offline when the real Secret only exists in the live cluster. Verify, cert, and proxy `secretRef`s still fail loud, since silently dropping verification or TLS material is a security downgrade.
